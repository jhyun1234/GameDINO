...
{
    //트리,새의 X가 공룡의 몸체쪽에 있을때,
    //공룡의 높이가 충분하지 않다면 충돌로 처리
    GotoXY(0, 0);
    printf("treeX : %d,birdX : %d, dinoY : %d", treeX, birdX, dinoY);
    if (treeX <= 8 && treeX >= 4 && birdX <= 5 && birdX >= 3 &&
        dinoY > 8)
    {
        return true;
    }
...
int main()
{
    ...
    while (true)        //(v2.0) 게임 루프
    {
        ...
        while (true)    //한 판에 대한 루프
        {
            ...
            if (isCollision(treeX, dinoY, birdX))
                break;
